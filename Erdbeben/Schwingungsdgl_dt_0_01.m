...
for t=0:dt1:T
  mstern = m+dt1*(1.0-1.0)*d+0.5*dt1*dt1*(1.0-2.0*0.5)*c;  
  ...
  % Newmark Werte
  fstern = ft-c*(x0_imp+dt1*v0_imp+dt1*dt1*0.5*a0_imp)-d*(v0_imp+dt1*1.0*a0_imp);
  if(abs(mstern)<1e-15)
	break;
  end
  ...
  v1 = v0_imp+dt1*((1.0-1.0)*a0_imp+1.0*a1);
  Spv = v1/vf;
  
  % aktuelle Position und bezogene Relativposition
  x1 = x0_imp+dt1*v0_imp+0.5*dt1*dt1*((1.0-2.0*0.5)*a0_imp+2.0*0.5*a1);
...
for t=0:dt1:T
  mstern = m+dt1*(1.0-0)*d+0.5*dt1*dt1*(1.0-2.0*0)*c;  
  ...
  % Newmark Werte
  fstern = ft-c*(x0_exp+dt1*v0_exp+dt1*dt1*0*a0_exp)-d*(v0_exp+dt1*0*a0_exp);
  if(abs(mstern)<1e-15)
	break;
  end
  ...
  % aktuelle Geschwindikeit und bezogene Relativgeschwindigkeit
  v1 = v0_exp+dt1*((1.0-0)*a0_exp+0*a1);
  Spv = v1/vf;
  
  % aktuelle Position und bezogene Relativposition
  x1 = x0_exp+dt1*v0_exp+0.5*dt1*dt1*((1.0-2.0*0)*a0_exp+2.0*0*a1);
  Spx = x1/xf ; 		%klassische Vergrößerungsfunktion
  Tout = Tout+dt1;
...
for t=0:dt1:T
  mstern = m+dt1*(1.0-gamma)*d+0.5*dt1*dt1*(1.0-2.0*beta)*c;  
  ...
  % Newmark Werte
  fstern = ft-c*(x0_newmark+dt1*v0_newmark+dt1*dt1*beta*a0_newmark)-d*(v0_newmark+dt1*gamma*a0_newmark);
  if(abs(mstern)<1e-15)
	break;
  end
  ...
  % aktuelle Geschwindikeit und bezogene Relativgeschwindigkeit
  v1 = v0_newmark+dt1*((1.0-gamma)*a0_newmark+gamma*a1);
  Spv = v1/vf;
  
  % aktuelle Position und bezogene Relativposition
  x1 = x0_newmark+dt1*v0_newmark+0.5*dt1*dt1*((1.0-2.0*beta)*a0_newmark+2.0*beta*a1);
  Spx = x1/xf ; 		%klassische Vergrößerungsfunktion
  Tout = Tout+dt1;
...
title(sprintf('Einfluss der Newmark Parameter beta (b) und gamma (g) fuer dt = %1.2f', dt1)); 		% title of plot
